...
# ==========================================
# TICKER CONVERTER (Institutional Rules)
# ==========================================
...
def main():
    ...
    print("Downloading Official Universe...")
    sapi.download_dataset("signals/v1.0/live.parquet", "live.parquet")
    ...
    # 2. Targeted Sampling (Top 250 US Stocks for Stability)
    ...
    # 4. SPECTACULAR ALPHA: Risk-Adjusted Mean Reversion (LdP Logic)
    print("Calculating Institutional Alpha...")
    df = df.with_columns([
    ...
    # Z-Score inverted for Mean Reversion, then divided by Annual Vol (Risk Parity)
    df = df.with_columns(
    ...
    if spread < 0:
        print("Yield Inversion Detected: Neutralizing Portfolio.")
    ...
    model_id = sapi.get_models()[MODEL_SLOT_NAME]
