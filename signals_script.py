...
def get_risk_multiplier():
    try:
        ...
        # Using a direct data pull to avoid further library bugs
        spread_data = web.get_data_fred('T10Y2Y', start, datetime.datetime.now())
        latest_spread = spread_data.iloc[-1].values[0]
        
        print(f"Current 10Y-2Y Spread: {latest_spread}")
        return 0.5 if latest_spread < 0 else 1.0
    except Exception as e:
        print(f"FRED link warning: {e}. Defaulting to safe risk (1.0).")
        return 1.0

# 3. ALPHA GENERATION (MScFE 610 Mean Reversion)
...
if __name__ == "__main__":
    print("Executing V4.1 Macro-Systemic Pipeline...")
    ...
    try:
        ...
        print("SUCCESS: V4.1 DEPLOYED AND SUBMITTED.")
    except Exception as e:
